...
unsafe extern "C" {
    /// Initialize the Gmsh API. This must be called before any call to the other functions in the
    /// API. If `argc' and `argv' (or just `argv' in Python or Julia) are provided, they will be
    /// handled in the same way as the command line arguments in the Gmsh app. If `readConfigFiles'
    /// is set, read system Gmsh configuration files (gmshrc and gmsh-options). If `run' is set,
    /// run in the same way as the Gmsh app, either interactively or in batch mode depending on the
    /// command line arguments. If `run' is not set, initializing the API sets the options
